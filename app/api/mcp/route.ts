  ...
  server.registerTool(
    "inspect_page",
    {
      description:
        ...
        "usable selector for each. Call this before record_demo to ground step selectors in " +
        "what's actually on the page, instead of guessing.",
      inputSchema: { url: httpUrl.describe("The page to inspect") },
      annotations: { readOnlyHint: true },
    },
    async ({ url }) => {
      try {
        const { title, elements } = await inspectPage(url);
